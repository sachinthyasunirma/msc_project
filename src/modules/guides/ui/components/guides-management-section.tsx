...
export function GuidesManagementSection({
  ...
  const guideScopedResources: GuideResourceKey[] = useMemo(
    () => ["guide-rates", "guide-languages", "guide-coverage-areas", "guide-licenses", "guide-certifications", "guide-documents", "guide-weekly-availability", "guide-blackout-dates", "guide-assignments"],
    []
  );
  ...
  const visibleFields = useMemo(
    () => isGuideManageMode && guideScopedResources.includes(resource) ? fields.filter((field) => field.key !== "guideId") : fields,
    [fields, guideScopedResources, isGuideManageMode, resource]
  );

  const visibleResources = useMemo(() => (!isGuideManageMode ? (["guides", "languages"] as GuideResourceKey[]) : guideScopedResources), [guideScopedResources, isGuideManageMode]);
  const managedGuide = useMemo(() => guides.find((guide) => String(guide.id) === managedGuideId) ?? null, [guides, managedGuideId]);
  ...
  const openDialog = (mode: "create" | "edit", row?: Record<string, unknown>) => {
    ...
    if (isGuideManageMode && guideScopedResources.includes(resource)) next.guideId = managedGuideId;
    setForm(next);
  ...
  const onDelete = async (row: Record<string, unknown>) => {
    if (!row.id) return;
    const targetLabel = String(row.code ?? "").trim() || String(row.fullName ?? "").trim() || String(row.name ?? "").trim() || String(row.id);
    ...
    <Card>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              ...
              {isGuideManageMode && managedGuide ? ` Managing: ${managedGuide.code} - ${managedGuide.fullName}` : ""}
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            {isGuideManageMode ? <Button variant="outline" asChild><Link href="/master-data/guides">Back to Guides</Link></Button> : null}
            <Button variant="outline" className="master-refresh-btn" onClick={() => void refreshAll()}>
              <RefreshCw className="mr-2 size-4" />
              Refresh
            </Button>
            {!isGuideManageMode && resource === "guides" ? <Button variant="outline" onClick={() => setBatchOpen(true)}>Batch Upload</Button> : null}
        ...
        <Tabs value={resource} onValueChange={(value) => setResource(value as GuideResourceKey)}>
          <div className="master-tabs-scroll">
            <TabsList className="master-tabs-list">
              {visibleResources.map((key) => <TabsTrigger key={key} value={key} className="master-tab-trigger">{META[key].title}</TabsTrigger>)}
            </TabsList>
          </div>
        </Tabs>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              {COLUMNS[resource].map((column) => <TableHead key={column.key}>{column.label}</TableHead>)}
                ...
                <TableRow key={String(row.id)}>
                  {COLUMNS[resource].map((column) => (
                    <TableCell key={column.key}>
                      {typeof row[column.key] === "boolean" ? (
                        <Badge variant={getBooleanMeta(column.key, Boolean(row[column.key])).active ? "default" : "secondary"}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => openDialog("edit", row)}><Edit3 className="size-4" /></Button>
                      <Button size="sm" variant="outline" onClick={() => void onDelete(row)}><Trash2 className="size-4" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))
        ...
        {!loading && records.length > 0 ? (
          <TablePagination totalItems={records.length} page={currentPage} pageSize={pageSize} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} />
        ) : null}
      </CardContent>
          ...
          <div className="grid max-h-[62vh] grid-cols-1 gap-3 overflow-x-hidden overflow-y-auto px-1 md:grid-cols-2">
            {visibleFields.map((field) => (
              <div key={field.key} className={`min-w-0 space-y-2 ${field.type === "json" ? "md:col-span-2" : ""}`}>
                ...
                {field.type === "select" ? (
                  <Select value={String(form[field.key] ?? (field.nullable ? "__none__" : ""))} onValueChange={(value) => setForm((prev) => ({ ...prev, [field.key]: value === "__none__" ? "" : value }))}>
                  ...
                  <div className="flex h-9 items-center justify-between rounded-md border px-3">
                    <span className="text-muted-foreground text-xs">{getBooleanMeta(field.key, Boolean(form[field.key])).text}</span>
                  ...
                  <Input type={field.type === "number" ? "number" : field.type === "datetime" ? "datetime-local" : "text"} value={String(form[field.key] ?? "")} onChange={(e) => setForm((prev) => ({ ...prev, [field.key]: e.target.value }))} />
                )}
              </div>
            ))}
          </div>
          ...
          lookupHints: [
            {
              ...
              values: currencies.map((item) => String(item.code ?? "").trim().toUpperCase()).filter((value) => value.length > 0).slice(0, 20),
            },
          ],
        }}
        readOnly={isReadOnly}
