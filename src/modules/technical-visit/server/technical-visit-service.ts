...
async function getAccess(headers: Headers) {
  ...
  if (!session?.user) {
    throw new TechnicalVisitError(401, "UNAUTHORIZED", "You are not authenticated.");
  }
  ...
  if (!user.companyId) {
    throw new TechnicalVisitError(403, "COMPANY_REQUIRED", "User is not linked to a company.");
  }
