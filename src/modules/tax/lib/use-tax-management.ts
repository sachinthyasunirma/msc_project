...
import {
  buildTaxRecordsParams,
  taxKeys,
} from "@/modules/tax/lib/tax-query";
import {
  createTaxRecord,
  deleteTaxRecord,
  listTaxRecords,
...
export function useTaxManagement({
  ...
  const createTaxMutation = useMutation({
    mutationFn: ({ targetResource, payload }: { targetResource: string; payload: Record<string, unknown> }) =>
      createTaxRecord(targetResource, payload),
  });
  const updateTaxMutation = useMutation({
    ...
    }) => updateTaxRecord(targetResource, id, payload),
  });
  const deleteTaxMutation = useMutation({
    mutationFn: ({ targetResource, id }: { targetResource: string; id: string }) =>
      deleteTaxRecord(targetResource, id),
  });

  const taxes = lookupsData?.taxes ?? EMPTY_ROWS;
  const jurisdictions = lookupsData?.jurisdictions ?? EMPTY_ROWS;
  ...
  useEffect(() => {
    ...
    notify.error(lookupsError instanceof Error ? lookupsError.message : "Failed to load tax lookup data.");
  }, [lookupsError]);
    ...
    try {
      ...
      visibleFields.forEach((field) => {
        ...
        if (field.type === "number") payload[field.key] = value === "" ? null : Number(value);
          ...
          ["code", "countryCode", "currencyCode", "taxCode", "jurisdictionCode"].includes(field.key) &&
          typeof value === "string"
        ) {
      ...
      if (dialog.mode === "create") {
        await createTaxMutation.mutateAsync({ targetResource: resource, payload });
  ...
  const onDelete = useCallback(
    ...
    [confirm, deleteTaxMutation, isReadOnly, refreshAll, resource]
  );
