...
export const POST = withApiLogging(
  { route: "/api/itineraries/[itineraryId]/exports", method: "POST", feature: "itinerary" },
  async (request: Request, context: ApiRouteContext<{ itineraryId: string }>) => {
    try {
      const params = await context.params;
      const payload = await request.json();
      const result = await createItineraryExportHook(params?.itineraryId || "", payload, request.headers);
