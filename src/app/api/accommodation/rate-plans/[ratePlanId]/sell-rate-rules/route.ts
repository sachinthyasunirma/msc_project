...
import {
  createHotelSellRateRule,
  listHotelSellRateRules,
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const result = await listHotelSellRateRules(
      ratePlanId,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await createHotelSellRateRule(ratePlanId, payload, request.headers);
