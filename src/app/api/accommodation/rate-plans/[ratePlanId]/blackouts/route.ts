...
import {
  createHotelRateBlackout,
  listHotelRateBlackouts,
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const result = await listHotelRateBlackouts(
      ratePlanId,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await createHotelRateBlackout(ratePlanId, payload, request.headers);
