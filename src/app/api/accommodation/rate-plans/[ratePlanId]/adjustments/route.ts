...
import {
  createHotelRateAdjustment,
  listHotelRateAdjustments,
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const result = await listHotelRateAdjustments(
      ratePlanId,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await createHotelRateAdjustment(ratePlanId, payload, request.headers);
