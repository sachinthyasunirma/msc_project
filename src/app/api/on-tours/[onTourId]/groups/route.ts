...
import { createOnTourSubgroupRecord, toOnTourErrorResponse } from "@/modules/on-tour/server/on-tour-service";

const postHandler = withApiLogging(
  { route: "/api/on-tours/[onTourId]/groups", feature: "on-tour" },
  async (request: Request, context: ApiRouteContext<{ onTourId: string }>) => {
    try {
      ...
      const created = await createOnTourSubgroupRecord(onTourId, body, request.headers);
